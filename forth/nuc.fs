...
[ 32000000 17 12 */ ] constant WB_CLOCK_FREQ

h# F000 constant uart-addr
h# F001 constant uart-state-addr
h# F002 constant io-wait-addr

: byte-to-read? ( -- f ) uart-state-addr @ d# 2 and d# 0 > ;

: io-wait ( -- ) h# 0 io-wait-addr ! ;

: key ( -- ch ) byte-to-read? 0= if io-wait then uart-addr @ ;

: emit-uart ( ch -- ) uart-addr ! ;
